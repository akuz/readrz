...
public class Program {
	...
	public static void main(String[] args) throws Exception {
		...
		String usageString = 
			...
			"   -ontologyDir string      : Path to ontology directory\n";

		String  mongoServer = null;
		Integer mongoPort = null;
		String  mongoDb = null;
		String  ontologyDir = null;
		...
		try {
			...
			if (args != null) {
				for (int i=0; i < args.length; i++) {
					...
					} else if ("-ontologyDir".equals(args[i])) {
						if (i+1 < args.length) {
							ontologyDir = StringUtils.unquote(args[i+1]);
							i++;
						}
					}
				}
			...
			if (ontologyDir == null) {
				throw new IllegalArgumentException("Ontology directory not specified");
			}
